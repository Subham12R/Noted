...
export function AIPanel({ editor, pageId, isOpen, onClose, initialMode = 'answer' }: AIPanelProps) {
  ...
  useEffect(() => {
    function handleOpenAIPanel(e: CustomEvent<{ mode: AIMode; context?: string }>) {
    ...
    return () => {
      window.removeEventListener('openAIPanel', handleOpenAIPanel as EventListener)
    }
  }, [])
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      } else if (['summarize', 'expand', 'improve', 'translate'].includes(mode)) {
  ...
  const requestEdit = useCallback((action: 'insert' | 'replace') => {
    if (!response) return
  ...
  const approveEdit = useCallback(() => {
    ...
    if (pendingEdit.action === 'insert') {
      ...
      const { from, to } = editor.state.selection
      if (from !== to) {
        editor.chain().focus().deleteRange({ from, to }).insertContent(pendingEdit.content).run()
      } else {
        editor.chain().focus().insertContent(pendingEdit.content).run()
      }
    }
  ...
  const handleKeyDown = useCallback(
    ...
    [onClose, pendingEdit, rejectEdit, prompt, isLoading, aiAvailable, handleGenerate]
  )
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40 flex flex-col">
      <div
        className={`relative bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-700 overflow-hidden transition-all duration-300 ${
      ...
      >
        ...
        <div className={`${outputHeight} transition-all duration-300 relative`}>
          <div
            ref={responseRef}
            className="h-full overflow-y-auto px-4 py-4 text-zinc-200"
          >
            {isLoading && !response && (
              <div className="flex items-center justify-center h-full">
                <HugeiconsIcon icon={Loading03Icon} className="h-8 w-8 animate-spin text-zinc-500" />
              </div>
            )}
            ...
            {!isLoading && !error && !response && (
              <div className="flex flex-col items-center justify-center h-full text-zinc-500 text-center">
                ...
                <p className="text-xs mt-1">I can summarize, expand, improve, or translate</p>
              </div>
            )}
          ...
          {response && !pendingEdit && (
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-2 py-1.5 bg-zinc-800/95 backdrop-blur rounded-full border border-zinc-700 shadow-lg">
              <button
                type="button"
                onClick={() => requestEdit('insert')}
                className="px-2.5 py-1 text-xs bg-purple-600 hover:bg-purple-500 text-white rounded-full font-medium transition-colors"
              >
                Insert
              </button>
              <button
                ...
                className="px-2.5 py-1 text-xs bg-zinc-700 hover:bg-zinc-600 text-white rounded-full font-medium transition-colors"
              >
                Replace
              </button>
        ...
        {pendingEdit && (
          <div className="absolute inset-0 bg-zinc-900/95 backdrop-blur flex items-center justify-center z-20">
        ...
        <form onSubmit={handleSubmit} className="border-t border-zinc-700 bg-zinc-800 px-3 py-3">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 px-3 py-2 bg-transparent text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none disabled:opacity-50"
            />
            ...
            <button
              ...
              className="p-2 text-zinc-400 hover:text-white hover:bg-zinc-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition-colors"
            >
              {isLoading ? (
